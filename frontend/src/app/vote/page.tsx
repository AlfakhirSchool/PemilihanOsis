...
export default function VotePage() {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center px-4">
        <form onSubmit={handleLogin} className="w-full max-w-sm space-y-4 rounded-xl bg-white p-8 shadow-lg">
          <h1 className="text-center text-xl font-bold text-biru">Pemilihan Ketua OSIS</h1>
          ...
          <button
            disabled={loading}
            className="w-full rounded-lg bg-biru p-3 font-semibold text-white disabled:opacity-50"
          >
  ...
  if (stage === 'konfirmasi' && selected) {
    return (
      <Center>
        ...
        <div className="mt-4 rounded-xl border-2 border-biru p-6">
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button
            onClick={submitVote}
            disabled={loading}
            className="flex-1 rounded-lg bg-biru p-3 font-semibold text-white disabled:opacity-50"
          >
  ...
  return (
    <main className="min-h-screen px-4 py-10">
      <h1 className="mb-6 text-center text-xl font-bold text-biru">{election?.title || 'Pilih Ketua OSIS'}</h1>
      ...
      <div className="mx-auto grid max-w-4xl gap-4 sm:grid-cols-2">
        {election?.candidates.map((c) => (
          <button
            ...
            className="rounded-xl bg-white p-6 text-left shadow hover:ring-2 hover:ring-biru"
          >
            <p className="text-sm font-semibold text-oranye">No. Urut {c.nomorUrut}</p>
